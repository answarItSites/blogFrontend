...
const HealthyEating = () => {
  ...
  useEffect(() => {
    fetchHealthyEatingBlogs();
  }, []);

  const fetchHealthyEatingBlogs = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5002/api/postArticle?department=Healthy Eating"
      );
      setBlogs(response.data.data);
      setLoading(false);
    } catch (error) {
      toast.error("Error fetching Healthy Eating blogs");
    ...
    return (
      <section className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center py-8">
          <p className="text-gray-500 text-lg">No Healthy Eating blogs found</p>
        </div>
      </section>
    );
  }
