...
const MealsBlogSection = () => {
  ...
  const fetchMealKitsBlogs = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5002/api/postArticle?department=Meal Kits"
      );
      setBlogs(response.data.data);
      setLoading(false);
    } catch (error) {
      toast.error("Error fetching Meal Kits blogs");
    ...
    return (
      <section className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center py-8">
          <p className="text-gray-500 text-lg">No Meal Kits blogs found</p>
        </div>
      </section>
    );
  }
...
export default MealsBlogSection;
